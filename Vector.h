...
public:
	...
	void choice()
	{
		...
		case 1:
			vx = v1x + v2x;
			vy = v1y + v2y;
			cout << "Сумма (" << vx << "," << vy << ")" << endl;
			break;
		case 2:
			vx = v1x - v2x;
			vy = v1y - v2y;
			cout << "Вычитание (" << vx << "," << vy << ")" << endl;
			break;
		}
	}
	...
	friend ostream& operator<<(ostream & out, vector & vec) {
		out << "Модуль:" << vec.mod << endl;
		out << "Первый вектор: " << vec.v1x << "," << vec.v1y << ";" << endl;
		out << "Второй вектор: " << vec.v2x << "," << vec.v2y << ";" << endl;
		return out;
	}
	...
	friend istream& operator>>(istream & in, vector & vec) {
		...
		cout << "y = ";
		in >> vec.v2y;
		cout << "Операция, которую вы хотите сделать:" << endl;
		cout << "1. Найти сумму." << endl;
		cout << "2. Найти вычитание." << endl;
		cout << "Операция: ";
		in >> vec.opera;
		return in;
	}
